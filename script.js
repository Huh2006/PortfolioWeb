...
/* Hero right sketch — JS-driven reveal (reliable cross-env) */
setTimeout(() => {
...
(function initTrail() {
  ...
  (function loop() {
    ...
    strokes.forEach(s => {
      pencilLine(s.x1, s.y1, s.x2, s.y2, s.life, s.w);
      s.life *= 0.982;
    });
...
(function drawHeroSketch() {
  ...
  // No fill properties — omitting fill means no fill in Rough.js
  ...
  let t = 0;
  function draw(fn, delay) { setTimeout(fn, delay); }

  /* — Outer ring (light, airy) */
  draw(() => rc.circle(W/2, H/2, 370, { ...iOpt, roughness: 2.2, strokeWidth: 1, stroke: '#CCCAC4' }), t += 0);
  /* — Inner accent circle */
  draw(() => rc.circle(W/2, H/2, 190, { ...gOpt, strokeWidth: 1.3 }), t += 130);
  /* — Monitor screen */
  draw(() => rc.rectangle(148, 148, 184, 128, { ...iOpt, strokeWidth: 1.3 }), t += 110);
  /* — Screen inner highlight line */
  draw(() => rc.line(162, 165, 318, 165, { ...iOpt, roughness: 1, strokeWidth: 0.8, stroke: '#CCCAC4' }), t += 60);
  /* — Monitor stand */
  draw(() => rc.line(240, 276, 240, 306, { ...iOpt, strokeWidth: 1.1 }), t += 70);
  draw(() => rc.line(205, 306, 275, 306, { ...iOpt, strokeWidth: 1.1 }), t += 50);
  /* — Code text on screen */
  draw(() => {
    ...
    (function fadeIn(a) {
      ...
      if (a < 1) requestAnimationFrame(() => fadeIn(Math.min(a + 0.05, 1)));
      else ctx.globalAlpha = 1;
    })(0);
  }, t += 110);
  /* — Floating dots */
  draw(() => {
    [[82, 92, 4.5, warm, 0.65], [392, 72, 3, green, 0.6], [62, 355, 3.5, ink, 0.25],
     [405, 348, 2.5, warm, 0.55], [265, 58, 2, green, 0.5]].forEach(([x, y, r, c, a]) => {
      ctx.beginPath();
  ...
  draw(() => {
    ...
    ctx.fillStyle = warm; ctx.fillText('✦', 74, 198);
    ctx.font = '14px Caveat, cursive';
    ctx.fillStyle = green; ctx.fillText('✦', 374, 272);
    ctx.globalAlpha = 1;
  }, t += 90);
  /* — Tagline */
  draw(() => {
    ...
    ctx.fillText('build. learn. repeat.', 128, 345);
    ctx.globalAlpha = 1;
  }, t += 130);
})();
...
function drawAbout(canvas) {
  ...
  /* label */
  d(() => {
    ctx.font = '400 17px Caveat, cursive';
    ctx.fillStyle = ink;
    ctx.globalAlpha = 0.28;
    ctx.fillText('my workspace ☕', 78, 305);
    ctx.globalAlpha = 1;
  }, t += 100);
}
...
$$('.project-card').forEach((card) => {
  ...
  card.addEventListener('mouseenter', () => {
    ...
    rc.rectangle(5, 5, canvas.width - 10, canvas.height - 10, {
      roughness: 2.5,
      stroke: isFeatured ? 'rgba(247,245,240,0.5)' : 'rgba(247,245,240,0.4)',
      strokeWidth: 1.5,
    });
    rc.rectangle(10, 10, canvas.width - 20, canvas.height - 20, {
      roughness: 3,
      stroke: isFeatured ? 'rgba(247,245,240,0.2)' : 'rgba(247,245,240,0.15)',
      strokeWidth: 1,
    });
